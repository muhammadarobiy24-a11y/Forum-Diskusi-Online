...
export default function PostsPage() {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* Layout: kolom kiri scroll, kolom kanan fixed */}
      <div className="flex flex-1 min-h-0 overflow-hidden">

        {/* ── KIRI: scrollable ─────────────────────────────── */}
        <div className="flex-1 min-w-0 overflow-y-auto">
          <div className="max-w-[760px] mx-auto px-4 py-4 xl:px-6 xl:py-6 flex flex-col gap-4">
        ...
        {/* ── KANAN: fixed, tidak scroll ───────────────────── */}
        <div className="hidden xl:flex flex-col w-80 shrink-0 overflow-y-auto border-l"
          style={{ borderColor: "var(--forum-sidebar-border, #e2c9ad)", background: "var(--forum-sidebar-bg, #f0ddc6)" }}>
          <div className="p-4 flex flex-col gap-4">
            ...
            <BentoCard accent={{ bg: "#fff4ed", border: "#ffd5b4" }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-orange-400">Statistik</p>
              </div>
            ...
            {user && (
              <BentoCard accent={{ bg: "#edfff5", border: "#b6f5d3" }}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-xs font-bold uppercase tracking-widest text-emerald-500">Kontribusi</p>
                </div>
            ...
            <BentoCard accent={{ bg: "#edf6ff", border: "#b3d9ff" }}>
              <div className="flex items-center gap-2 mb-4">
                ...
                <p className="text-xs font-bold uppercase tracking-widest text-blue-400">Trending</p>
              </div>
              {[80, 62, 74, 55, 68].map((w, i) => (
                <div key={i} className="flex items-center gap-3 py-2.5 border-b last:border-0"
                  style={{ borderColor: "#b3d9ff" }}>
                  <span className="text-xs font-black forum-gradient-text w-5 shrink-0">#{i + 1}</span>
                  <div className="flex-1 h-2.5 rounded-full animate-pulse"
                    style={{ width: `${w}%`, background: "#dbeafe" }} />
                </div>
              ))}
              <p className="text-[10px] text-[var(--forum-text-muted)] text-center mt-3 italic">Segera hadir</p>
            </BentoCard>
